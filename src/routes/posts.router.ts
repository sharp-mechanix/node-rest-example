...
import { CreatePostRequestDto, ListPostsQueryDto, UpdatePostRequestDto } from '../contracts/posts';
...
router.post('/posts', async (request: Request, response: Response) => {
    const newPostRequest = request.body as CreatePostRequestDto;

    response.json(await controller.createPost(newPostRequest));
});

router.put('/posts', async (request: Request, response: Response) => {
    const updatePostRequest = request.body as UpdatePostRequestDto;
